---
title: Metrics for the unmeasurable
date: '2019-01-26T14:10:37-06:00'
format: link
service: instapaper
tags:
- read
external_url: https://lethain.com/metrics-for-the-unmeasurable/
---
...
![](https://i0.wp.com/lethain.com/static/blog/2019/synthetic-metrics.png?w=607&ssl=1)![](https://i0.wp.com/lethain.com/static/blog/2019/synthetic-metrics.png?w=607&ssl=1)
...
On the smallest teams, decision-makers might be deep in the code on a daily basis. A bit larger, and you’re talking about tasks in sprints. More growth, and you’re talking about collections of tasks, and adopting fancy words like “epics.” At a hundred-plus engineers, you’re likely talking primarily in themes of work with focus on a couple [key initiatives](https://lethain.com/infrastructure-planning/). At thousands, you might be talking about your investment framework for resourcing decisions.

No matter the path, metrics are your final destination.

Metrics are a remarkably effective abstraction.  

A [well-formed metric](https://lethain.com/goals-and-baselines/) has enough context to understand how well you’re doing, contextualize that performance against historical performance, and define a clear, gradable goal.

An example metric with a goal might be:

> 20% of deploys were [canaried](https://martinfowler.com/bliki/CanaryRelease.html) in Q4, up from 10% in Q3, but missing our goal of 25%.
...
* How quickly are libraries with [CVEs](https://cve.mitre.org/) upgraded?
* How old is your oldest active server?
* What percent of requests go over mTLS?
* How many folks have SSH access to servers?
* How many folks have root access to servers?
...
1. It should expose enough gradations to detect changes in performance over time,
2. changes across gradations should guide useful action,
3. they should be evaluable with limited context, and
4. they should be sufficiently broad to abstract significant complexity.
...
As an aside, measuring productivity is challenging for all the same reasons as measuring security. I’ve written a bit about this before, both summarizing [Accelerate’s definition of productivity](https://lethain.com/accelerate-developer-productivity/) which focuses on using industry research to select the *right* input metrics, and in my [introduction to systems thinking](https://lethain.com/systems-thinking/) which tries to summarize input metrics into a comprehensible model.
