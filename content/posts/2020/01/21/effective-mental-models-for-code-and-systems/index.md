...
date: '2020-01-21T23:09:34-07:00'
format: link
service: instapaper
tags:
- read
external_url: https://medium.com/@copyconstruct/effective-mental-models-for-code-and-systems-7c55918f1b3e
---
...
![](https://miro.medium.com/fit/c/96/96/2*Mb4iezdIzZIfBSgjulILnQ.jpeg)![](https://miro.medium.com/fit/c/96/96/2*Mb4iezdIzZIfBSgjulILnQ.jpeg)
Cindy Sridharan
Dec 30, 2018 · 16 min read



*As always, I’m greatly indebted to* [*Fred Hebert*](https://twitter.com/mononcqc?lang=en) *for reviewing a draft of this post and offering some invaluable insights.*

A little over a month ago, [Carmen Andoh](https://twitter.com/carmatrocity) gave [a talk at a meetup](https://twitter.com/copyconstruct/status/1064191067501678593) in Copenhagen about visualizations, though it really was a talk on mental models. In her talk, Carmen described a mental model as:
...
Carmen also drew attention to what [Rob Pike](https://twitter.com/rob_pike?lang=en) had once said about Ken Thompson’s preferred approach to debugging:
...
In the excellent book [***A Philosophy of Software Design***](https://www.amazon.com/Philosophy-Software-Design-John-Ousterhout/dp/1732102201/ref=sr_1_2?ie=UTF8&qid=1546206275&sr=8-2&keywords=a+philosophy+of+software+design), [John Ousterhout](https://twitter.com/johnousterhout?lang=en) writes:
...
Fortunately, for us, this isn’t all doom and gloom. As [John Allspaw](https://twitter.com/allspaw) writes in an incredibly insightful recent article titled “[**Recalibrating Mental Models Through Design of Chaos Experiments**](https://www.dropbox.com/s/6dv5n3zhv1ozt3w/infoq.pdf?dl=0)**”**:
...
A programmer modifying the code doesn’t have to arrive at the *exact same mental model* under which the code was written; they only need to arrive ata *close enough approximation* of it. A quintessential example of this is “[the relativity of wrong](https://chem.tufts.edu/answersinscience/relativityofwrong.htm)” proposed by Isaac Asimov, which uses different wrong models for the curvature of the earth (the earth is flat, spherical, oblate spheroid) to demonstrate how this does not necessarily prevent everyday uses of the model. The model becomes “wrong” only when an invariant is changed.
...
Much has been written about the pros and cons of [self-documenting code](https://en.wikipedia.org/wiki/Self-documenting_code). In my experience, the problem with code that’s self-documenting is that it doesn’t allow for the dissemination of the *context* under which the code was written. Code is the end result of a team understanding a problem, communicating with users, and working towards an artifact that’s something of a compromise between various competing constraints and requirements. The code itself lets a reader know *what* decisions were made, but not *why*.
...
Well-designed codebases are often composed in layers and subsystems, with each layer providing as close to hermetic an abstraction as possible ([as impossible as it might seem](https://www.joelonsoftware.com/2002/11/11/the-law-of-leaky-abstractions/)) while exposing a simple interface to its users.

However, while designing layers, it’s also important to note that the composition of these layers can significantly shape the understandability of the codebase as a whole. The decomposition of software into layers should be a reflection of the fact that the most powerful mental models are intuitive *and* modular*,* hierarchicalyet amenable to a breadth-first exploration.
...
In his talk [The Do’s and Don’ts of Error Handling](https://youtu.be/TTM_b7EJg5E?t=1331), [Joe Armstrong](https://twitter.com/joeerl?lang=en) posits that “large assemblies of small things are impossible to prove correct”, and that when it comes to large systems, it becomes important to be able to identify and isolate the *error kernel* of a system.
...
Quoting from the article “[**Recalibrating Mental Models Through Design of Chaos Experiments**](https://www.dropbox.com/s/6dv5n3zhv1ozt3w/infoq.pdf?dl=0)**”** again:
...
A recent [New Yorker profile](https://www.newyorker.com/magazine/2018/12/10/the-friendship-that-made-google-huge) on Google’s famed duo Jeff Dean and Sanjay Ghemawat features a paragraph on Barbara Liskov’s views on programming:
